...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'paymentDueDate' | 'paymentPaidDate'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      paymentDueDate: payment.paymentDueDate?.toJSON() ?? null,
      paymentPaidDate: payment.paymentPaidDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      paymentDueDate: restPayment.paymentDueDate ? dayjs(restPayment.paymentDueDate) : undefined,
      paymentPaidDate: restPayment.paymentPaidDate ? dayjs(restPayment.paymentPaidDate) : undefined,
    };
  }
