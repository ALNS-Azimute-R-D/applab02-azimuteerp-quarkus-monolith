...
import { sampleWithRequiredData } from '../locality.test-samples';
...
describe('Locality Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [
        RouterTestingModule.withRoutes([{ path: 'locality', component: LocalityComponent }]),
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.localities?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to localityService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getLocalityIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getLocalityIdentifier).toHaveBeenCalledWith(entity);
