...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'placedDate' | 'estimatedDeliveryDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...
      estimatedDeliveryDate: order.estimatedDeliveryDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...
      estimatedDeliveryDate: restOrder.estimatedDeliveryDate ? dayjs(restOrder.estimatedDeliveryDate) : undefined,
    };
  }
