...
describe('Category e2e test', () => {
  ...
  const categorySample = { name: 'strip joke' };
  ...
  describe('new Category page', () => {
    ...
    it('should create an instance of Category', () => {
      cy.get(`[data-cy="acronym"]`).type('upward upon wow');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'upward upon wow');

      cy.get(`[data-cy="name"]`).type('end');
      cy.get(`[data-cy="name"]`).should('have.value', 'end');

      cy.get(`[data-cy="description"]`).type('gadzooks');
      cy.get(`[data-cy="description"]`).should('have.value', 'gadzooks');

      cy.get(`[data-cy="handlerClazzName"]`).type('ick harmless late');
      cy.get(`[data-cy="handlerClazzName"]`).should('have.value', 'ick harmless late');
