...
describe('PaymentMethod e2e test', () => {
  ...
  const paymentMethodSample = { code: 'afore', description: 'jovially but' };
  ...
  describe('new PaymentMethod page', () => {
    ...
    it('should create an instance of PaymentMethod', () => {
      cy.get(`[data-cy="code"]`).type('consequent');
      cy.get(`[data-cy="code"]`).should('have.value', 'consequent');

      cy.get(`[data-cy="description"]`).type('or when interest');
      cy.get(`[data-cy="description"]`).should('have.value', 'or when interest');

      cy.get(`[data-cy="businessHandlerClazz"]`).type('yahoo ack');
      cy.get(`[data-cy="businessHandlerClazz"]`).should('have.value', 'yahoo ack');
